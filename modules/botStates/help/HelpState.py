...
class HelpState(BaseState):
    ...
    def receive_message(self, message: Message, user_data: dict):
        if message.text == self.main_button[0]:
            self.send("В настоящее время этот раздел разрабатывается. Если есть какие-то вопросы напишите изх через поддержку заявок, с вами свяжется администратор и напишет ответ на вопроснапишите через подачу заявок.")
